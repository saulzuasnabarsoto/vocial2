defmodule Vocial2Web.ChatChannel do
  use Vocial2Web, :channel

  alias Vocial2.Votes
  alias Vocial2Web.Presence
  ...
  def handle_in("user_active", %{"username" => username}, socket) do
    ...
    if meta.status == "idle" do
      ...
      push socket, "presence_diff", Presence.list(socket)
      {:ok, _} = Presence.update(socket, username, %{online_at: inspect(System.system_time(:second)), status: "active"})
    end
    {:noreply, socket}
  end
  ...
  def handle_info(:after_join, socket) do
    ...
    {:ok, _} = Presence.track(socket, socket.assigns.username, %{online_at: inspect(System.system_time(:second)), status: "active"})
    {:noreply, socket}
  end
end
